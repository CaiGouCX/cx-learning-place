...
def sixstep_sinusoidal(mode):
    ...
    control.set_direction(+1)#"forward"
    #control.set_direction(-1)#"reverse"

    pid = PID(kp=5, ki=0.8, kd=0.05, dt=dt, output_limits=(-220, 220))
    setpoint = motor.direction*314.0
    ...
    for i in range(steps):
        ...
        va, vb, vc = control.compute_voltage(theta, omega,omega_target=setpoint)
...
def foc():
    ...
    for i in range(steps):
        ...
        va, vb, vc = control.compute_voltage(theta_e=theta, omega=omega, omega_target=setpoint, ia=ia, ib=ib, ic=ic)
